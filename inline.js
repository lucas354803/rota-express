...
async function entrar(){
  ...
  if(tipo==="admin" && normalizarTexto(user)==="admin" && senha==="P@loma3548"){
...
async function gerarPixMercadoPago(id){
  ...
  try{
    ...
    await db.from("pedidos").update({mp_payment_id: String(data.id || ""), pagamento_status: data.status === "approved" ? "Pix confirmado" : "Aguardando confirmação", liberado: data.status === "approved" ? true : p.liberado, status: data.status === "approved" ? "Liberado para motoboys" : "Aguardando pagamento Pix"}).eq("id", p.id);
...
async function verificarPixPendentesAutomatico(){
  if(!sessao || !Array.isArray(pedidos)) return;
  ...
  let confirmouAlgum = false;
  for(const p of pendentes.slice(0, 5)){
    const ok = await verificarPix(p.id, true);
    confirmouAlgum = confirmouAlgum || ok;
  }

  if(confirmouAlgum){
    await carregarDados();
    render();
    if(sessao.tipo === "admin" || sessao.tipo === "motoboy"){
      tocarSomNovaEntrega();
    }
  }
}
...
async function criarPedidoCliente(){
  ...
  await carregarDados();
  render();
  alert("Pedido criado. Gere/pague o Pix. Assim que o Pix cair, a entrega aparece automaticamente para os motoboys.");
}
...
async function pagarMotoboy(id){
  ...
  const pagamento = {
    motoboy_id:id,
    motoboy_nome:m.nome,
    valor_pago:Number(liquido.toFixed(2)),
  ...
  await carregarDados();
  setAbaAdmin('pagamentos');
  alert("Pagamento registrado no Histórico de pagamentos e saldo do motoboy zerado.");
}
...
function adminHTML(){
  ...
  const conteudo = abaAdmin === "pedidos" ? `
    <h2>Pedidos para aprovar</h2>${pedidos.map(cardPedidoAdmin).join("") || "<p>Nenhum pedido.</p>"}` :
    abaAdmin === "entregas" ? `
...
function cardPedidoAdmin(p){
  return `<div class="order">
    ...
    <div class="row">
      <button class="yellow" onclick="confirmarPix(${p.id})">Confirmar Pix manual</button>
      <button class="green" onclick="verificarPix(${p.id})">Verificar Pix</button>
...
async function iniciar(){
  ...
  intervaloAtualizacao = setInterval(async () => {
    ...
    await carregarDados();
    await verificarPixPendentesAutomatico();
