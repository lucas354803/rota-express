...
export default async function handler(req, res) {
  ...
  if (!token) return res.status(500).json({ error: "Token Mercado Pago não configurado", message: "Configure MP_TOKEN na Vercel." });
  ...
  try {
    ...
    if (pedido.mp_payment_id) {
      const r = await fetch(`https://api.mercadopago.com/v1/payments/${pedido.mp_payment_id}`, { headers: { Authorization: `Bearer ${token}` } });
      payment = await r.json();
      if (!r.ok) payment = null;
    }

    if (!payment) {
      const r = await fetch(`https://api.mercadopago.com/v1/payments/search?external_reference=${encodeURIComponent(pedidoId)}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    // Só libera automaticamente se ainda não foi aceita/finalizada por motoboy.
    if (approved && !pedido.motoboy_id && !isFinalizada(pedido.status)) {
      patch.liberado = true;
