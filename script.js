...
let abaMotoboy = localStorage.getItem("rota_aba_motoboy") || "corridas";
...
function statusNormalizado(p){return String((p && p.status) || "").trim().toLowerCase();}
function isFinalizada(p){return statusNormalizado(p).includes("finalizada") || statusNormalizado(p).includes("entregue");}
function isRecusado(p){return statusNormalizado(p).includes("recusado");}
function idIgual(a,b){return String(a ?? "") === String(b ?? "");}
function pagamentoMotoboyPendente(p){return isFinalizada(p) && !p.pago_motoboy;}
function entregaAtivaMotoboy(p, motoboyId){return idIgual(p.motoboy_id, motoboyId) && !isFinalizada(p) && !isRecusado(p);}
function entregaLiberadaSemMotoboy(p){return !!p.liberado && p.pagamento_status === "Pix confirmado" && !p.motoboy_id && !isFinalizada(p) && !isRecusado(p);}
...
async function carregarDados(){
  ...
  historicoEntregas = ent.error ? pedidos.filter(isFinalizada) : [...(ent.data || []), ...pedidos.filter(isFinalizada).filter(p => !(ent.data || []).some(h => String(h.pedido_id || h.id) === String(p.id)))];
...
async function verificarPix(id, silencioso=false){
  try{
    ...
    render();
    if(!silencioso){
      alert(data.approved ? "Pix confirmado! A entrega foi liberada automaticamente para os motoboys." : "Pagamento ainda não confirmado.");
    }
...
async function verificarPixPendentesAutomaticamente(){
  const pendentes = pedidos.filter(p =>
    p.mp_payment_id &&
    p.pagamento_status !== "Pix confirmado" &&
    !isFinalizada(p) &&
    !isRecusado(p)
  ).slice(0, 5);

  for(const p of pendentes){
    await verificarPix(p.id, true);
  }
}
...
async function criarPedidoCliente(){
  ...
  await carregarDados();
  render();
  alert("Pedido criado. Gere o Pix, pague e o sistema vai liberar automaticamente quando confirmar.");
}
...
async function mudarStatus(id,status){
  ...
  if(error){alert("Erro: " + error.message); return;}
  if(status === "Entrega finalizada"){
    const p = pedidos.find(x => idIgual(x.id, id));
    if(p){
      p.status = "Entrega finalizada"; p.liberado = false; p.data_finalizacao = extra.data_finalizacao;
      const jaExiste = await db.from("historico_entregas").select("id").eq("pedido_id", id).limit(1);
      if(!jaExiste.data || jaExiste.data.length === 0){
        const ins = await db.from("historico_entregas").insert({
          ...
          valor:Number(p.valor || 0), km:Number(p.km || 0), finalizado_em:extra.data_finalizacao
        });
        if(ins.error){ console.warn("Histórico não gravou, mas pedido foi finalizado:", ins.error.message); }
      }
    }
    if(sessao && sessao.tipo === "motoboy"){ abaMotoboy = "relatorio"; localStorage.setItem("rota_aba_motoboy", "relatorio"); }
  }
  await carregarDados(); render();
}
...
async function pagarMotoboy(id){
  ...
  const corridas = pedidos.filter(p=>idIgual(p.motoboy_id,id) && pagamentoMotoboyPendente(p));
...
async function deletarMotoboy(id){
  ...
  if(!m) return;
  const temCorridaAtiva = pedidos.some(p => idIgual(p.motoboy_id,id) && !isFinalizada(p) && !p.pago_motoboy);
...
function clienteHTML(){
  ...
  return `
    ...
    <h2>Meus pedidos</h2>
    ${meus.some(p => p.status === "Entrega finalizada") ? `<div class="notice">✅ Você tem entrega marcada como entregue/finalizada.</div>` : ""}
...
function adminHTML(){
  ...
  let rota = pedidos.filter(p=>pagamentoMotoboyPendente(p)).reduce((t,p)=>t+(Number(p.valor || 0)*TAXA_ROTA/100),0);
  let aPagar = pedidos.filter(p=>pagamentoMotoboyPendente(p)).reduce((t,p)=>t+(Number(p.valor || 0)*(100-TAXA_ROTA)/100),0);
  const conteudo = abaAdmin === "pedidos" ? `
    <h2>Pedidos ativos</h2>${pedidos.filter(p=>!isFinalizada(p)).map(cardPedidoAdmin).join("") || "<p>Nenhum pedido ativo.</p>"}` :
    abaAdmin === "entregas" ? `
    <h2>Histórico de entregas</h2>${historicoEntregas.map(cardHistoricoEntrega).join("") || "<p>Nenhuma entrega finalizada.</p>"}` :
    abaAdmin === "pagamentos" ? `
...
function motoboyHTML(){
  let liberados = pedidos.filter(entregaLiberadaSemMotoboy);
  let minhasAtivas = pedidos.filter(p=>entregaAtivaMotoboy(p, sessao.id));
  let minhasFinalizadas = pedidos.filter(p=>idIgual(p.motoboy_id, sessao.id) && isFinalizada(p));
  let finalizadasEmAberto = minhasFinalizadas.filter(p=>!p.pago_motoboy);
  let bruto = finalizadasEmAberto.reduce((t,p)=>t+Number(p.valor || 0),0);
  let taxa = bruto*TAXA_ROTA/100;
  let liquido = bruto-taxa;
  let conteudo = "";
  if(abaMotoboy === "relatorio"){
    conteudo = '<h2>Relatório de entregas</h2>' + (minhasFinalizadas.map(cardPedidoMotoboyRelatorio).join("") || "<p>Nenhuma entrega finalizada.</p>");
  } else {
    conteudo = '<h2>Corridas liberadas</h2>' + (liberados.map(cardPedidoMotoboy).join("") || "<p>Nenhuma corrida liberada.</p>") + '<h2>Minhas corridas</h2>' + (minhasAtivas.map(cardPedidoMotoboy).join("") || "<p>Nenhuma corrida sua em andamento.</p>");
  }
  return `
    <div class="card">
    ...
    <div class="stats">
      <div class="stat"><span>Finalizadas a receber</span><b>${finalizadasEmAberto.length}</b></div>
    ...
    <div class="tabs">
      <button class="${abaMotoboy==='corridas'?'yellow':'gray'}" onclick="setAbaMotoboy('corridas')">Minhas corridas</button>
      <button class="${abaMotoboy==='relatorio'?'yellow':'gray'}" onclick="setAbaMotoboy('relatorio')">Relatório de entregas</button>
    </div>
    ${conteudo}
  `;
}
...
function cardPedidoCliente(p){
  const entregue = isFinalizada(p) ? `<div class="notice">✅ Entrega entregue/finalizada pelo motoboy.</div>` : "";
...
function cardPedidoAdmin(p){
  return `<div class="order">
    ...
    <div class="row">
      <button class="yellow" onclick="verificarPix(${p.id})">Verificar Pix</button>
...
function cardPedidoMotoboy(p){
  return `<div class="order">
    ...
    ${idIgual(p.motoboy_id, sessao.id) && !isFinalizada(p)?`
...
function cardPedidoMotoboyRelatorio(p){
  return `<div class="order">
    <b>#${p.id}</b> <span class="badge">Entrega finalizada</span>
    ${baseCard(p)}
    <p><b>Finalizado em:</b> ${p.data_finalizacao || ""}</p>
    <p><b>Pagamento:</b> ${p.pago_motoboy ? "Pago" : "Aguardando pagamento do admin"}</p>
  </div>`;
}
...
function cardMotoboyAdmin(m){
  let abertos = pedidos.filter(p=>idIgual(p.motoboy_id,m.id) && pagamentoMotoboyPendente(p));
...
async function iniciar(){
  ...
  intervaloAtualizacao = setInterval(async () => {
    ...
    if(sessao.tipo === "admin" || sessao.tipo === "cliente"){
      await verificarPixPendentesAutomaticamente();
    }
