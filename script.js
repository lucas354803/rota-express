...
function clienteHTML(){
  ...
  return `
    ...
    <h2>Meus pedidos</h2>
    ${meus.some(p => statusFinalizado(p.status)) ? `<div class="notice">✅ Você tem entrega marcada como entregue/finalizada.</div>` : ""}
...
function motoboyHTML(){
  ...
  let meus = pedidos.filter(p=>motoboyDoPedido(p, sessao) && !statusFinalizado(p.status));
  let finalizadosHoje = corridasParaReceberMotoboy(sessao.id);
  let bruto = finalizadosHoje.reduce((t,p)=>t+valorPedido(p),0);
