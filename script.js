...
async function carregarDados(){
  ...
  historicoEntregas = ent.error ? pedidos.filter(p => p.status === "Entrega finalizada") : (ent.data || []);
    ...
    if(p){
      ...
      if(!jaExiste.data || jaExiste.data.length === 0){
        await db.from("historico_entregas").insert({
          ...
          valor:Number(p.valor || 0), km:Number(p.km || 0), finalizado_em:agoraBR()
        });
      }
    }
  }
...
async function deletarMotoboy(id){
  ...
  if(!m) return;
  const temCorridaAtiva = pedidos.some(p => String(p.motoboy_id) === String(id) && p.status !== "Entrega finalizada" && !p.pago_motoboy);
...
function adminHTML(){
  ...
  const conteudo = abaAdmin === "pedidos" ? `
    <h2>Pedidos ativos</h2>${pedidos.filter(p=>p.status!=="Entrega finalizada").map(cardPedidoAdmin).join("") || "<p>Nenhum pedido ativo.</p>"}` :
    abaAdmin === "entregas" ? `
...
function motoboyHTML(){
  let liberados = pedidos.filter(p=>p.liberado && p.pagamento_status==="Pix confirmado" && !p.motoboy_id && p.status!=="Entrega finalizada");
  let minhasAtivas = pedidos.filter(p=>String(p.motoboy_id)===String(sessao.id) && p.status!=="Entrega finalizada");
  let minhasFinalizadas = pedidos.filter(p=>String(p.motoboy_id)===String(sessao.id) && p.status==="Entrega finalizada");
  ...
  return `
    ...
    ${minhasAtivas.map(cardPedidoMotoboy).join("") || "<p>Nenhuma corrida sua em andamento.</p>"}
    <h2>Relatório de entregas</h2>
    ${minhasFinalizadas.map(cardPedidoMotoboyRelatorio).join("") || "<p>Nenhuma entrega finalizada.</p>"}
  `;
}
...
function cardPedidoCliente(p){
  const entregue = p.status === "Entrega finalizada" ? `<div class="notice">✅ Entrega entregue/finalizada pelo motoboy.</div>` : "";
...
function cardPedidoMotoboy(p){
  return `<div class="order">
    ...
    ${String(p.motoboy_id)===String(sessao.id) && p.status!=="Entrega finalizada"?`
