...
function corridasParaReceberMotoboy(motoboyId){
  const porPedido = pedidos.filter(p =>
    idIgual(p.motoboy_id, motoboyId) &&
  ...
  const porHistorico = historicoEntregas.filter(h =>
    idIgual(h.motoboy_id, motoboyId) &&
...
async function deletarMotoboy(id){
  const m = motoboys.find(x => idIgual(x.id,id));
  if(!m) return;
  const temCorridaAtiva = pedidos.some(p => idIgual(p.motoboy_id,id) && p.status !== "Entrega finalizada" && !p.pago_motoboy);
...
function adminHTML(){
  ...
  let rota = corridasAbertas.reduce((t,p)=>t+(Number(p.valor || 0)*TAXA_ROTA/100),0);
  let aPagar = corridasAbertas.reduce((t,p)=>t+(Number(p.valor || 0)*(100-TAXA_ROTA)/100),0);
...
function motoboyHTML(){
  ...
  let meus = pedidos.filter(p=>idIgual(p.motoboy_id, sessao.id) && p.status!=="Entrega finalizada");
  let finalizadosHoje = corridasParaReceberMotoboy(sessao.id);
